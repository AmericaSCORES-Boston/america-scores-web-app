...
1. Install the latest version of Node. `brew install node` on Mac if you have Homebrew installed or follow the instructions [here](https://nodejs.org/en/) for Mac or Windows. To ensure you installed it correctly, run `node -v` and `npm -v`, making sure they output a version number.

2. Clone the project from Git. CD into the directory.

3. Install the dependencies. Run `npm install` in the project directory (at the same level as package.json) and make sure it builds successfully.

4. To ensure everything went smoothly, run `npm start` and go to [http://localhost:3000](http://localhost:3000). If you don't see anything loaded, let Charlie know!
